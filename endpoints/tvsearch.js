                ...
                item: results.map(({show, id, nzbName}) => (
                    {
                        ...
                        link: `${getBaseUrl(req)}/api?mode=nzb-download&pid=${id}&season=${season}&ep=${ep}&apikey=${req.query.apikey}`,
                        enclosure: {$:{url : `${getBaseUrl(req)}/api?mode=nzb-download&pid=${id}&season=${season}&ep=${ep}&apikey=${req.query.apikey}`, length : "2147483648", type: "application/x-nzb"} } 
                      }
                ))
            }
        }
